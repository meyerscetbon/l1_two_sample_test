...
def test_asymptotic(X,Y,test_locs,gwith2,alpha):
    ...
    z_1 = gauss_kernel(X,test_locs, gwith2)
    cov_1 = np.cov(z_1.T)


    z_2 = gauss_kernel(Y,test_locs,gwith2)
    cov_2 = np.cov(z_2.T)
    ...
    results = {'alpha': alpha,
    'pvalue': p_value,
    'h0_rejected': p_value < alpha,'test': S}
...
#return T, and gwidth
    ...
    def gain_torch(param):
        ...
        D2 = D2 - 2 * torch.matmul(X_new,torch.t(torch.reshape(param[:J*d], (J, d))))
        ...
        D2 = torch.reshape(D2,(D2.shape[0],1))
        D2 = D2 - 2 * torch.matmul(Y_new,torch.t(torch.reshape(param[:J*d], (J, d))))
        D2 = D2 + torch.sum(torch.reshape(param[:J*d],(J,d))**2,1)
        ...
        if J>1:
            ...
            S = (torch.sqrt(torch.Tensor([t]).double()))*(torch.inverse(square_root).matmul(S))
    ...
    for t in range(max_iter):
        ...
        param[:J*d]=param[:J*d]+T_step_size*grad_T/(t+1)**step_pow/np.sum(grad_T**2)**step_pow

        update_gwidth = gwidth_step_size * np.sign(grad_gwidth) * min(np.abs(grad_gwidth), max_gam_sq_step)/(t+1)**step_pow
...
def test_asymptotic_SCF(X,Y,test_locs,gwith2,alpha):
    ...
    results = {'alpha': alpha,
    'pvalue': p_value,'h0_rejected': p_value < alpha, 'test': S}
...
def solver_SCF(X,Y,n_test_locs,max_iter=300,
        T_step_size=1, gwidth_step_size=0.1,
        tol_fun=1e-4):
    ...
    def gain_torch(param):
        ...
        x_freq = torch.matmul(X_scale,torch.reshape(param[:J*d],(J,d)).t()).reshape(n,J)
        y_freq = torch.matmul(Y_scale,torch.reshape(param[:J*d],(J,d)).t()).reshape(m,J)
    ...
    for t in range(max_iter):
        ...
        try:
            ...
            param[:J*d]=param[:J*d]+T_step_size*grad_T/(t+1)**step_pow/np.sum(grad_T**2)**0.5
